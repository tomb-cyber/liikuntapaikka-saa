...
const options = {
    ...
    path: '/api/sports-places?fields=name&fields=location.geometries&page=1&pageSize=100',
    method: 'GET',
    headers: {
...
const getPlaces = () => {
    ...
    const perusGet = options => {
        return getNHandleJSON(options)
            .then((input) => {
                console.log(input.status)
                let filtered = input.obj.filter(each => each.location !== undefined)
    ...
    return new Promise((resolve, reject) => {
        ...
        const request = http.request(options, (response) => {
            ...
            response.on('end', () => {
                ...
                resolve({ obj: obj, status: response.statusCode })
            })
        })
...
const migrateAllToDB = () => {
    getPlaces().then(res => addToDbBulk(res.map(each => jsonToSchemaParser(each))))
...
const addToDbBulk = (schemaObjArray) => {
    Liikuntapaikka.collection.insertMany(schemaObjArray).then(() => {
        console.log('liikuntapaikka bulk saved!')
        mongoose.connection.close()
    })
}


const jsonToSchemaParser = (liikuntapaikka) => {
    const obj = {
        location: liikuntapaikka.location,
        name: liikuntapaikka.name,
        sportsPlaceId: liikuntapaikka.sportsPlaceId
    }

    return new Liikuntapaikka(obj)
}